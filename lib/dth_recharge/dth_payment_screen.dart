...
class DthPaymentScreen extends StatefulWidget {
  ...
  String paymentmode;
   ...
   DthPaymentScreen(this.plan,this.cardnumber,this.phonenumber,this.rechargeamount,this.paidamount,this.paymentmode) ;

  @override
  _DthPaymentScreenState createState() => _DthPaymentScreenState(this.plan,this.cardnumber,this.phonenumber,this.rechargeamount,this.paidamount,this.paymentmode);
...
class _DthPaymentScreenState extends State<DthPaymentScreen> {
  ...
  String paidamount;
  String paymentmode;
  String resulturl="https://mysaveapp.com/easyrecharge/paymentgateway/result.php";

  WeiplCheckoutFlutter wlCheckoutFlutter = WeiplCheckoutFlutter();
  _DthPaymentScreenState(this. plan, this. cardnumber, this. phonenumber, this. rechargeamount, this. paidamount,this.paymentmode);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Payment",style: TextStyle(fontSize: 14,color: Colors.white),),
  ...
  Future<void> handleResponse(dynamic response)  async {
    ...
    String url1= "https://mysaveapp.com/easyrecharge/updatePaymentdetailsToRecharge.php?timestamp="+new DateTime.now().microsecondsSinceEpoch.toString();
...
loadResult(String url1,String transactionId)async
{
      ...
      if (data.isNotEmpty) {
        ...
        if (res.isNotEmpty) {
          if (res.contains("Recharge Amount mismatch")) {
            ...
            _showNotification("Recharge process failed..", "Recharge Amount mismatch");
          } else {
            String s = res;
            ...
            if (status == "2") {
              _showNotification("Successfully completed your recharge..\n$msg", "Recharge");
  ...
  Future<void> _updateRechargeStatus(String rpid, String agentid, String status,String transactionId) async {
    // TODO: Implement API call or DB update
    debugPrint("Update Recharge Status: rpid=$rpid agentid=$agentid status=$status");
    ...
    String urldata="https://mysaving.in/IntegraAccount/api/updateRechargeStatus.php?timestamp="+DateTime.now().microsecondsSinceEpoch.toString();
  ...
  Future<void> updateGenStatus(String transactionId) async {
    ...
    String urldata="https://mysaving.in/IntegraAccount/api/updateRechargeStatusRetry.php?timestamp="+DateTime.now().microsecondsSinceEpoch.toString();
    ...
    if(entity.status==1)
      {
        ...
        String urldata1="https://mysaving.in/IntegraAccount/api/PostTransactionata.php";
      ...
      goToPaymentSection(id_transaction,entity.data!.emailId.toString(),entity.data!.mobile.toString(),rechargeamount);
  ...
  goToPaymentSection(String transactionid,String email,String phone,String amount)async
  {
    ...
    String urldata1="https://mysaving.in/IntegraAccount/ecommerce_api/getPaymentCredentials.php";
    ...
    String a = merchantcode + "|" + txnid + "|" + amount + "||" + customerid + "|"+phone+"|"+email+"||||||||||" + salt;



    String  methode = "https://mysaveapp.com/generateHash.php?timestamp="+new DateTime.now().microsecondsSinceEpoch.toString();
